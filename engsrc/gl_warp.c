...
void EmitWaterPolys (msurface_t *fa)
{
	...
	for (p=fa->polys ; p ; p=p->next)
	{
		...
		GLfloat WaterP[count*3];
		GLfloat WaterP_Tex[count*2];
		int ofsv = 0;
		int ofstx = 0;
		for (i=0,v=p->verts[0] ; i<p->numverts ; i++, v+=VERTEXSIZE)
		{
		...
		glVertexPointer(3, GL_FLOAT, 0, WaterP);
		glTexCoordPointer(2, GL_FLOAT, 0, WaterP_Tex);
...
void EmitSkyPolys (msurface_t *fa)
{
	...
	for (p=fa->polys ; p ; p=p->next)
	{
		...
		GLfloat SkyP[count*3];
		GLfloat SkyP_Tex[count*2];
		int ofsv = 0;
		int ofstx = 0;
		for (i=0,v=p->verts[0] ; i<p->numverts ; i++, v+=VERTEXSIZE)
		{
		...
		glVertexPointer(3, GL_FLOAT, 0, SkyP);
		glTexCoordPointer(2, GL_FLOAT, 0, SkyP_Tex);
		glDrawArrays(GL_TRIANGLE_FAN,0,count);
